...
from scipy.stats import linregress, entropy, skew, kurtosis
...
# per user aggregates with additional stats
user_inter_gap = sess_times.groupby('client_id')['inter_sess_gap_h'].agg(
    avg_inter_gap='mean',
    median_inter_gap='median',
    max_inter_gap='max',
    std_inter_gap='std',  # Add standard deviation for gap regularity
    cv_inter_gap=lambda x: x.std()/(x.mean() + 1e-6)  # Coefficient of variation
).fillna(0)
...
blocks=[user_evt, user_sess_dur,
    ...
    search_conv,         # search-to-buy conversion
    purchase_rate,
    carts_per_search,
    buys_per_search,
    search_abandon,
    ...
    user_daily, user_dow, queries_per_session,
    rate30, last_buy_days, user_hr, user_hour_cycle,
    user_pod, user_month, user_quarter, user_week,
    user_doy_cycle, peak, buy_q_frac, user_sess,
    sess_std, med_len, price_stats, user_cat, cat30,
    rec_div, user_name, cum, var, disc, user_pop,
    browse_div, page_visit_per_buy, event_div,
    depth_max, depth_avg, user_dec
]

# ─── FEATURE GROUPING ─────────────────────────────────────────────────────
# Temporal features group
temporal_features = pd.concat([
    user_hour_cycle,     # Hour of day cyclical features
    user_doy_cycle,      # Day of year cyclical features
    peak,               # Peak activity hour features
    buy_q_frac,         # Quarterly purchase distribution
    user_dow,           # Day of week features
    user_month,         # Monthly patterns
    user_quarter,       # Quarterly patterns
    user_week,          # Weekly patterns
    rate30,             # 30-day event rate
    last_buy_days,      # Days since last purchase
    activity_momentum,   # Activity trend
    activity_distribution, # Activity distribution stats
    weekly_pattern_strength # Weekly pattern strength
], axis=1)

# Behavioral features group
behavioral_features = pd.concat([
    evt_ent,            # Event type entropy
    cat_ent,            # Category entropy
    cat_gini,           # Category concentration
    browse_div,         # Browsing diversity
    event_div,          # Event type diversity
    depth_max,          # Max interaction depth
    depth_avg,          # Average interaction depth
    sku_div,            # SKU diversity
    sku_rec_wt,         # Recency-weighted SKU diversity
    user_pop,           # Average item popularity
    novelty             # Novelty seeking behavior
], axis=1)

# Conversion features group
conversion_features = pd.concat([
    cart_abandon,        # Cart abandonment rate
    search_conv,         # Search to purchase conversion
    purchase_rate,       # Overall purchase rate
    immediate_conv,      # Immediate conversion rate
    query_purchase_speed, # Search to purchase speed
    abandoned_sku_diversity, # Diversity of abandoned items
    price_abandon_rate,  # Price-based abandonment rates
    carts_per_search,   # Cart adds per search
    buys_per_search,    # Purchases per search
    search_abandon,
    log_counts,
    conversion_flags,
    count_feats,
...
# Session features group
session_features = pd.concat([
    user_sess_dur,      # Session duration stats
    user_inter_gap,     # Inter-session gaps
    pod_frac,           # Part of day distribution
    burst_ratio,        # Session burst ratio
    user_sess,          # Session counts and lengths
    sess_std,           # Session gap variability
    med_len,            # Median session length
    user_daily,         # Daily session stats
    queries_per_session # Queries per session
], axis=1)

# ─── FEATURE PREPROCESSING & DIMENSION REDUCTION ───────────────────────────
...
user_feats = pd.concat(feature_blocks, axis=1)

# Remove constant and quasi-constant features
selector = VarianceThreshold(threshold=0.01)  # Remove features with very low variance
user_feats_clean = pd.DataFrame(
    selector.fit_transform(user_feats),
    index=user_feats.index,
    columns=user_feats.columns[selector.get_support()]
)

# Handle infinite values and missing data
user_feats_clean.replace([np.inf, -np.inf], np.nan, inplace=True)
user_feats_clean.fillna(0, inplace=True)

# Use RobustScaler instead of StandardScaler for better handling of outliers
X_scaled = RobustScaler(with_centering=False).fit_transform(user_feats_clean)

# Apply SVD for dimension reduction
svd = TruncatedSVD(n_components=SVD_DIM, random_state=42)
emb_svd = svd.fit_transform(X_scaled).astype(np.float16)

# Calculate and print explained variance ratio
explained_var_ratio = svd.explained_variance_ratio_
cumulative_var_ratio = np.cumsum(explained_var_ratio)
print(f"Explained variance ratio: {cumulative_var_ratio[-1]:.3f}")

# Save results
client_ids = user_feats_clean.index.to_numpy(np.int64)
np.savez_compressed('user_features.npz',
...
# 1. Price Sensitivity Curve - how user behavior changes with price
price_sensitivity = []
for client_id in sku['client_id'].unique():
    client_data = sku[sku['client_id'] == client_id]
    if len(client_data) < 5:  # Need sufficient data
        price_sensitivity.append((client_id, 0, 0))
        continue
   
    # Create price bins and calculate purchase rate per bin
    client_data['price_bin'] = pd.qcut(client_data['price_n'],
                                      min(5, len(client_data['price_n'].unique())),
                                      labels=False,
                                      duplicates='drop')
    purchase_rates = []
    for bin_id in client_data['price_bin'].unique():
        bin_data = client_data[client_data['price_bin'] == bin_id]
        purchase_rate = (bin_data['event_type'] == 'product_buy').mean()
        purchase_rates.append((bin_id, purchase_rate))
   
    if len(purchase_rates) > 1:
        # Calculate slope of purchase rate vs price bin
        x = [p[0] for p in purchase_rates]
        y = [p[1] for p in purchase_rates]
        if len(x) > 1 and len(set(y)) > 1:  # Ensure variance in y
    ...
    # Calculate price elasticity: % change in purchase probability / % change in price
    if len(client_data) > 10:
        high_price = client_data['price_n'].quantile(0.75)
        low_price = client_data['price_n'].quantile(0.25)
        high_purchase_rate = (client_data[client_data['price_n'] >= high_price]['event_type'] == 'product_buy').mean()
        low_purchase_rate = (client_data[client_data['price_n'] <= low_price]['event_type'] == 'product_buy').mean()
       
        # Avoid division by zero
        if low_purchase_rate > 0 and low_price > 0:
            price_elasticity = ((high_purchase_rate - low_purchase_rate) / low_purchase_rate) / ((high_price - low_price) / low_price)
        else:
            price_elasticity = 0
    else:
        price_elasticity = 0
   
    price_sensitivity.append((client_id, slope, price_elasticity))

# Convert to DataFrame
price_sensitivity_df = pd.DataFrame(price_sensitivity,
                                   columns=['client_id', 'price_sensitivity_slope', 'price_elasticity'])
price_sensitivity_df.set_index('client_id', inplace=True)

# 2. Product Exploration Patterns - measure exploration vs. exploitation behavior
product_exploration = []
for client_id in sku['client_id'].unique():
    client_data = sku[sku['client_id'] == client_id]
    ...
    if len(client_data) < 5:
        product_exploration.append((client_id, 0, 0, 0))
        continue
   
    # Sort by timestamp
    client_data = client_data.sort_values('timestamp')
   
    # Calculate cumulative unique SKUs
    '''
    client_data['cumulative_unique'] = client_data['sku'].expanding().nunique()
    client_data['total_interactions'] = range(1, len(client_data) + 1)
    '''
    client_data['cumulative_unique'] = client_data['sku'].expanding().apply(pd.Series.nunique)
    client_data['total_interactions'] = range(1, len(client_data) + 1)

    # Exploration rate: rate of discovering new SKUs
    if len(client_data) > 5:
        x = client_data['total_interactions'].values
        y = client_data['cumulative_unique'].values
        # Use log function to model diminishing returns in exploration
        try:
            popt, _ = curve_fit(lambda x, a, b: a * np.log(x + b), x, y,
                              bounds=([0, 0], [np.inf, np.inf]))
            exploration_rate = popt[0]  # Higher means more exploration
        except:
            exploration_rate = client_data['sku'].nunique() / len(client_data)
    else:
        exploration_rate = client_data['sku'].nunique() / len(client_data)
   
    # Exploitation ratio: repeat interactions with same SKUs
    repeat_interactions = len(client_data) - client_data['sku'].nunique()
    exploitation_ratio = repeat_interactions / len(client_data) if len(client_data) > 0 else 0
   
    # Exploration decay: how quickly user settles on preferred products
    # (measure by percentage of new products in last 25% of interactions vs first 25%)
    if len(client_data) >= 8:  # Need enough data for meaningful quarters
        q1_data = client_data.iloc[:len(client_data)//4]
        q4_data = client_data.iloc[-len(client_data)//4:]
        q1_exploration = q1_data['sku'].nunique() / len(q1_data)
        q4_exploration = len(set(q4_data['sku']) - set(client_data.iloc[:-len(client_data)//4]['sku'])) / len(q4_data)
        exploration_decay = (q1_exploration - q4_exploration) / q1_exploration if q1_exploration > 0 else 0
    else:
        exploration_decay = 0
       
    product_exploration.append((client_id, exploration_rate, exploitation_ratio, exploration_decay))

# Convert to DataFrame
exploration_df = pd.DataFrame(product_exploration,
                             columns=['client_id', 'exploration_rate', 'exploitation_ratio', 'exploration_decay'])
exploration_df.set_index('client_id', inplace=True)

# 3. User Purchase Journey Analysis - funnel progression efficiency
journey_metrics = []
for client_id in ev['client_id'].unique():
    client_data = ev[ev['client_id'] == client_id].sort_values('timestamp')
   
    if len(client_data) < 5:  # Need sufficient data
        journey_metrics.append((client_id, 0, 0, 0, 0))
        continue
   
    # Define journey stages
    search_events = client_data[client_data['event_type'] == 'search_query']
    visit_events = client_data[client_data['event_type'] == 'page_visit']
    cart_events = client_data[client_data['event_type'] == 'add_to_cart']
    purchase_events = client_data[client_data['event_type'] == 'product_buy']
   
    # Calculate conversion efficiencies
    search_to_visit = min(1, len(visit_events) / len(search_events)) if len(search_events) > 0 else 0
    visit_to_cart = min(1, len(cart_events) / len(visit_events)) if len(visit_events) > 0 else 0
    cart_to_purchase = min(1, len(purchase_events) / len(cart_events)) if len(cart_events) > 0 else 0
   
    # Journey efficiency: compound metric of funnel progression
    journey_efficiency = search_to_visit * visit_to_cart * cart_to_purchase
   
    # Average number of steps before purchase
    if len(purchase_events) > 0:
        # For each purchase, calculate number of preceding events in the same session
        steps_before_purchase = []
        for purchase in purchase_events.itertuples():
            # Find session
            session_id = purchase.sess_id if 'sess_id' in purchase else None
            if session_id is not None:
                session_events = client_data[client_data['sess_id'] == session_id]
                purchase_idx = session_events.index.get_loc(purchase.Index)
                steps_before_purchase.append(purchase_idx)
       
        avg_steps = np.mean(steps_before_purchase) if steps_before_purchase else 0
    else:
        avg_steps = 0
   
    journey_metrics.append((client_id, journey_efficiency, search_to_visit, visit_to_cart, cart_to_purchase))

# Convert to DataFrame
journey_df = pd.DataFrame(journey_metrics,
                         columns=['client_id', 'journey_efficiency', 'search_to_visit_rate',
                                 'visit_to_cart_rate', 'cart_to_purchase_rate'])
journey_df.set_index('client_id', inplace=True)

# 4. Session progression patterns - how behavior evolves within sessions
session_progression = []
for client_id in seq['client_id'].unique():
    client_sessions = seq[seq['client_id'] == client_id]
   
    if len(client_sessions['sess_id'].unique()) < 3:  # Need multiple sessions
        session_progression.append((client_id, 0, 0, 0))
        continue
   
    session_stats = []
    for sess_id in client_sessions['sess_id'].unique():
        session_data = client_sessions[client_sessions['sess_id'] == sess_id].sort_values('timestamp')
       
        # Skip very short sessions
        if len(session_data) < 3:
            continue
           
        # Split session into thirds to analyze progression
        third_size = max(1, len(session_data) // 3)
        first_third = session_data.iloc[:third_size]
        last_third = session_data.iloc[-third_size:]
       
        # Calculate metrics for beginning vs end of session
        early_page_visit_rate = (first_third['event_type'] == 'page_visit').mean()
        late_page_visit_rate = (last_third['event_type'] == 'page_visit').mean()
       
        early_add_to_cart_rate = (first_third['event_type'] == 'add_to_cart').mean()
        late_add_to_cart_rate = (last_third['event_type'] == 'add_to_cart').mean()
       
        early_purchase_rate = (first_third['event_type'] == 'product_buy').mean()
        late_purchase_rate = (last_third['event_type'] == 'product_buy').mean()
       
        # Compare beginning vs end behavior
        page_visit_shift = late_page_visit_rate - early_page_visit_rate
        add_cart_shift = late_add_to_cart_rate - early_add_to_cart_rate
        purchase_shift = late_purchase_rate - early_purchase_rate
       
        session_stats.append((page_visit_shift, add_cart_shift, purchase_shift))
   
    # Average across sessions
    if session_stats:
        avg_page_shift = np.mean([s[0] for s in session_stats])
        avg_cart_shift = np.mean([s[1] for s in session_stats])
        avg_purchase_shift = np.mean([s[2] for s in session_stats])
    else:
        avg_page_shift, avg_cart_shift, avg_purchase_shift = 0, 0, 0
       
    session_progression.append((client_id, avg_page_shift, avg_cart_shift, avg_purchase_shift))

# Convert to DataFrame
progression_df = pd.DataFrame(session_progression,
                             columns=['client_id', 'page_visit_progression',
                                     'add_cart_progression', 'purchase_progression'])
progression_df.set_index('client_id', inplace=True)

# 5. Temporal shopping patterns - time between similar item interactions
temporal_patterns = []
for client_id in sku['client_id'].unique():
    client_data = sku[sku['client_id'] == client_id].sort_values('timestamp')
   
    if len(client_data) < 5:  # Need sufficient data
        temporal_patterns.append((client_id, 0, 0, 0))
        continue
   
    # Group by category and analyze intervals
    category_intervals = []
    for category in client_data['category'].unique():
        ...
        # Calculate time between consecutive interactions with same category
        intervals = cat_data['timestamp'].diff().dt.total_seconds() / (3600*24)  # in days
        intervals = intervals.dropna()
       
        if len(intervals) > 0:
            category_intervals.extend(intervals.tolist())
   
    # Calculate statistics on intervals
    if category_intervals:
        median_category_interval = np.median(category_intervals)
        cv_category_interval = np.std(category_intervals) / np.mean(category_intervals) if np.mean(category_intervals) > 0 else 0
       
        # Identify if user has weekly/monthly periodicity
        intervals_array = np.array(category_intervals)
        weekly_periodicity = np.sum((intervals_array > 6) & (intervals_array < 8)) / len(intervals_array)
        monthly_periodicity = np.sum((intervals_array > 28) & (intervals_array < 32)) / len(intervals_array)
        periodicity_score = max(weekly_periodicity, monthly_periodicity)
    else:
        median_category_interval, cv_category_interval, periodicity_score = 0, 0, 0
       
    temporal_patterns.append((client_id, median_category_interval, cv_category_interval, periodicity_score))

# Convert to DataFrame
temporal_df = pd.DataFrame(temporal_patterns,
                          columns=['client_id', 'median_category_interval',
                                  'category_interval_variability', 'shopping_periodicity'])
temporal_df.set_index('client_id', inplace=True)

# 6. User Segment Features - Create segment indicators based on behavior
segment_features = []

for client_id in ev['client_id'].unique():
    client_data = ev[ev['client_id'] == client_id]
   
    # Skip if not enough data
    if len(client_data) < 10:
        segment_features.append((client_id, 0, 0, 0, 0, 0))
        continue
   
    # Calculate segment indicators
   
    # Researcher: high search-to-purchase ratio
    search_events = sum(client_data['event_type'] == 'search_query')
    purchase_events = sum(client_data['event_type'] == 'product_buy')
    is_researcher = (search_events / (purchase_events + 1)) > 5
   
    # Browser: high page visits, low conversions
    page_visits = sum(client_data['event_type'] == 'page_visit')
    is_browser = (page_visits / len(client_data) > 0.7) and (purchase_events / (page_visits + 1) < 0.1)
   
    # Decisive: direct path to purchase, fewer steps
    cart_events = sum(client_data['event_type'] == 'add_to_cart')
    cart_to_purchase = purchase_events / (cart_events + 1)
    is_decisive = cart_to_purchase > 0.8
   
    # Deliberate: adds and removes items from cart frequently
    remove_events = sum(client_data['event_type'] == 'remove_from_cart')
    is_deliberate = (remove_events / (cart_events + 1)) > 0.3
   
    # Loyal: returns frequently
    if 'sess_id' in client_data.columns:
        session_count = client_data['sess_id'].nunique()
        days_active = (client_data['timestamp'].max() - client_data['timestamp'].min()).days + 1
        is_loyal = (session_count / (days_active + 1)) > 0.3  # Multiple sessions per day
    else:
        is_loyal = False
   
    segment_features.append((client_id,
                            int(is_researcher),
                            int(is_browser),
                            int(is_decisive),
                            int(is_deliberate),
...
segment_df = pd.DataFrame(segment_features,
                         columns=['client_id', 'is_researcher', 'is_browser',
                                 'is_decisive', 'is_deliberate', 'is_loyal'])
segment_df.set_index('client_id', inplace=True)
...
feature_blocks = [
    ...
    price_moments,
    activity_distribution,
    advanced_features  # Add the new advanced features
]
